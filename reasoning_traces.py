"""
Generate Reasoning Traces and SFT Dataset for Preference Learning

This script:
1. Loads baseline preference data from the Stanford SHP dataset
2. Generates reasoning traces using Together AI (Llama 3.3 70B)
3. Filters for agreements only (to avoid sycophancy)
4. Creates an SFT dataset with position randomization
"""
...
def load_shp_dataset(sample_size=None, split="train"):
    """
    Load the Stanford Human Preferences (SHP) dataset.
...
def generate_reasoning_traces(baseline_data, client, model=DEFAULT_MODEL, sample_size=None):
    ...
    Args:
        baseline_data: List of preference examples
...
def generate_sft_dataset(reasoning_data, output_file):
    ...
    (e.g., always preferring response A based on position rather than content).
